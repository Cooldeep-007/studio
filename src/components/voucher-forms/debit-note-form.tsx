...
export function DebitNoteForm() {
    ...
    // Ledgers that can be credited in a debit note, e.g., Purchase Return, Rate Difference etc.
    ...
    const companyState = "Karnataka"; // Mock company state

    const watchedLineItems = form.watch('lineItems');
    const placeOfSupply = form.watch('placeOfSupply');
    const partyLedgerId = form.watch('supplierLedgerId');
     ...
     React.useEffect(() => {
        ...
        // Avoid infinite loop by checking for actual changes
                ...
                <Card>
                    ...
                    <CardContent className="space-y-6">
                        ...
                        <div>
                            ...
                            <TableBody>
                                {fields.map((field, index) => (
                                    <TableRow key={field.id}>
                                        ...
                                        <TableCell><FormField control={form.control} name={`lineItems.${index}.uqc`} render={({ field }) => (
                                        ...
                                        <TableCell><FormField control={form.control} name={`lineItems.${index}.gstRate`} render={({ field }) => (
                          ...
                          <Button type="button" variant="outline" size="sm" className="mt-4" onClick={() => append({ itemName: '', quantity: 1, rate: 0, taxableValue: 0, gstRate: 0, cgst: 0, sgst: 0, igst: 0, total: 0, hsnCode: '', uqc: '' })}>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                            ...
                            <div className="w-full space-y-2 self-end">
                                ...
                                <div className="flex justify-between text-sm text-muted-foreground"><span>Input CGST Reversal</span><span>{watchedLineItems.reduce((acc, item) => acc + item.cgst, 0).toFixed(2)}</span></div>
                                <div className="flex justify-between text-sm text-muted-foreground"><span>Input SGST Reversal</span><span>{watchedLineItems.reduce((acc, item) => acc + item.sgst, 0).toFixed(2)}</span></div>
                                <div className="flex justify-between text-sm text-muted-foreground"><span>Input IGST Reversal</span><span>{watchedLineItems.reduce((acc, item) => acc + item.igst, 0).toFixed(2)}</span></div>
                                <Separator />
                                <div className="flex justify-between font-bold text-lg"><span>Total Debit Note Value</span><span>{form.getValues('grandTotal').toFixed(2)}</span></div>
                            </div>
                        </div>
                    </CardContent>
                    <CardFooter>
