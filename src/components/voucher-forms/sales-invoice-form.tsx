...
export function SalesInvoiceForm({ initialData }: SalesInvoiceFormProps) {
    ...
    const handleItemCreated = (newItem: Item) => {
        ...
        if (lastIndex >= 0 && !form.getValues(`items.${lastIndex}.itemId`)) {
            ...
            append({ itemId: newItem.id, quantity: 1, rate: newItem.unitPrice, discount: 0 });
        }
    };
                                ...
                                <TableBody>
                                    {fields.map((field, index) => {
                                        ...
                                        <TableRow key={field.id} className="align-top">
                                            <TableCell><FormField control={form.control} name={`items.${index}.itemId`} render={({ field }) => (<Combobox options={itemOptions} {...field} placeholder="Select item..." onChange={(value) => { field.onChange(value); handleItemSelection(value, index); }} />)} /></TableCell>
